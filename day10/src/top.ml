...
let create scope ({ clock; reset_n; uart_rx } : _ I.t) : _ O.t =
  ...
  let solution =
    Solution.create scope { clock; reset_n; ascii_char = receiver.rx_byte }
  in
  let num_to_display = solution.fewest_button_presses in
  let display =
...
let hierarchical scope =
  ...
  Scoped.hierarchical ~scope ~name:"day10_top" create
;;
